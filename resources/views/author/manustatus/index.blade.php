    ...
    <div class="py-12 w-full">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg p-2">
                <span class="text-xl px-4 py-2">Manuscript Status Panel</span>
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Author Info.</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Manuscript Info.</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($reviewedstatuses as $reviewedstatus)
                                        @if (Auth::user()->id == $reviewedstatus->user_id)
                                        <tr class="">
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class=" items-center">
                                                    {{ $reviewedstatus->au_id }} - {{ $reviewedstatus->au_fname }} {{ $reviewedstatus->au_lname }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class=" items-center">
                                                    {{ $reviewedstatus->manu_id }} - ( {{ $reviewedstatus->manuscript_id }} )
                                                </div>
                                            </td>
                                             <td class="px-6 py-4">
                                                <div class=" items-center bg-green-500 text-white rounded pl-5">
                                                    {{ $reviewedstatus->rev_stat_status }}
                                                </div>
                                            </td>
                                        </tr>
                                        @endif
